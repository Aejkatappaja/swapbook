...
- **`--insecure` flag.** Skips TLS certificate verification for the target, so a
  dev app behind a self-signed certificate (a local reverse proxy, mkcert, an
  internal CA) can be previewed instead of failing the handshake. Covers all
  three paths to the target: the reverse proxy, the overlay's own adapter
  fetches, and `swapbook check`.
